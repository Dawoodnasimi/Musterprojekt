...
const handleDeleteClick = (posts) => {
  ...
  //const lasturl = lastTwoElements[1].url;
  //console.log(posts.data.length);

  posts.links.forEach((url) => {
    //console.log(url);
    //console.log("number of data" + posts.data.length);
    const lasturl = posts.links[posts.links.length - 1].url;
    console.log(lasturl);

    if (url["url"] === null && posts.data.length === 1) {
      console.log("last pag");
    }
  });
};
