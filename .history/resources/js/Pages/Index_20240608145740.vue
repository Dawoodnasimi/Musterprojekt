...
const message = ref($page.props.flash.success || $page.props.flash.error);
const type = ref($page.props.flash.success ? "success" : "error");
...
<template>
  ...
  <!--   <p class="text-3xl font-bold">{{ $page.props.flash.success }}</p>
 -->

  <transition name="fade">
    <div
      v-if="message"
      :class="`fixed bottom-5 right-5 flex items-center bg-white shadow-lg rounded-lg px-4 py-2 ${
    ...
    >
      ...
      >
        <i
          class="fas"
          :class="`${type === 'success' ? 'fa-check-circle' : 'fa-exclamation-circle'}`"
        ></i>
      </div>
      <div class="text-sm">
        {{ message }}
          ...
          >
            ...
            >
              ...
              <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                ...
                <button
                  type="button"
                  class="bg-white-200 text-black mt-2 mx-1 px-3 py-2 rounded-md text-sm font-medium hover:bg-gray-300 border border-gray-400 hover:border-gray-500"
                  @click="showConfirmation = false"
                >
                  Cancel
                </button>
