...
const search = () => {
    if (searchTerm.value) {
        ...
        if (my_data.value.length === 0) {
            my_data.value = props.posts.data;
            console.log("it's zero" + my_data.value);
        } else {
            my_data.value = my_data.value;
            console.log("it's not zero" + my_data.value);
        }
    } else if (
        my_data.value.length === 0 ||
        my_data.value === null ||
        my_data.value === undefined ||
        my_data.value === ""
    ) {
        console.log("there's no data to search for");
    } else {
        my_data.value = props.posts.data;
        console.log("it's the else condition" + my_data.value);
    }
};
