...
const handleDeleteClick = async (post) => {
  ...
  // Assuming you have a deletePost function that deletes the post
  await deletePost(post);

  // Fetch the posts again after deleting
  await fetchPosts();

  // Check if there are no posts on the current page
  if (posts.value.data.length === 0 && currentPage.value > 1) {
    currentPage.value--;
    // Fetch the posts again for the previous page
    await fetchPosts();
  }
};
            ...
            >
              ...
              >
                <Link
                  ...
                  class="bg-red-500 text-white mt-2 mx-1 px-3 py-2 rounded-md text-sm font-small hover:bg-red-600 mr-2"
