<script setup>
import { defineProps, ref, defineComponent, reactive } from "vue";
...
const posts = reactive(this.posts.map((post) => ({ ...post, showFullText: false })));

function toggleFullText(post) {
  post.showFullText = !post.showFullText;
}

export { posts, toggleFullText, showConfirmation, postToDelete, confirmDelete };
</script>
  ...
  <div class="mx-auto p-5 grid grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-3">
    ...
    >
      ...
      <p
        ...
        :class="{ 'truncate-text': !post.showFullText }"
        v-on:click="toggleFullText(post)"
      >
        {{ post.content }}
      </p>
      <transition name="fade">
        <p v-if="post.showFullText" class="text-gray-500 pb-2">
          {{ post.content }}
        </p>
      </transition>
      <strong
