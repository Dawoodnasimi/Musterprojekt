...
const hasPrevious = computed(() => {
  return posts.links.some((link) => link.label === "Previous" && link.url !== null);
});

const hasNext = computed(() => {
  return posts.links.some((link) => link.label === "Next" && link.url !== null);
});

const previousPageUrl = computed(() => {
  const previousLink = posts.links.find((link) => link.label === "Previous");
  return previousLink ? previousLink.url : null;
});

const nextPageUrl = computed(() => {
  const nextLink = posts.links.find((link) => link.label === "Next");
