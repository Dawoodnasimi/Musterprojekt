...
const fetchPosts = async (page = 1) => {
  const response = await fetch(`/posts?page=${page}`); // Pass the page number to the API endpoint
  const data = await response.json();
  posts.value = data;
};

// Watch for changes in the page query parameter and fetch the posts for the new page
watchEffect(() => {
