...
const handleDeleteClick = (posts) => {
  ...
  posts.links.forEach((link) => {
    ...
    if (link.url === null && posts.data.length === 1 && previousUrl) {
...
// Use onMounted to get localStorage data when the component is mounted
onMounted(() => {
  getlocalStorage();
});
...
watchEffect(() => {
  ...
  if (message.value) {
    setTimeout(() => {
      message.value = "";
      type.value = "";
      localStorage.clear(); // Clear localStorage only when the toast disappears
    }, 5000);
  }
});
</script>
