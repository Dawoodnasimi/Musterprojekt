...
/* const message = ref(props.flash.success || toast.value.message);
const type = ref(props.flash.success ? "success" : toast.value.type);
...
// Use watchEffect to sync message and type with toast data
watchEffect(() => {
  ...
  if (message.value || props.flash.success || toast.value.message) {
    setTimeout(() => {
      message.value = "";
      type.value = "";
      localStorage.removeItem("toast"); // Clear only the toast item
    }, 5000);
  }
});
</script>
...
<template>
  ...
  <div class="container mx-auto p-5 w-full flex justify-end items-center relative">
    <h1 class="text-3xl font-bold absolute left-1/2 transform -translate-x-1/2">
      {{ message }}
    </h1>
    <Link
